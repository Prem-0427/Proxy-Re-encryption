...
def UploadFiles():
    if request.method=='POST':
        ...
        # print('KKKKKKKKKKKKKKKKKKKKKKKKK',key)
        print('AAAAAAAAAAAAAAAAAA', Keywords)

        sql="insert into filesupload (doemail,FileName,Keywords,Files,AccessPilicy,Date,Time,aesKey) values (%s,%s,%s,AES_ENCRYPT(%s,%s),%s,%s,%s,%s)"
        values=(session['doemail'],FileName,Keywords,data,Keywords,Police,current_date,t,Keywords)
...
def acceptfile(id=0):
    ...
    skey = secrets.token_hex(4)
    print("secret key", skey)
    mail_content =f'Your request is accepted by Admin and email is:{reemail} and The Your secrate key is: {otp}' 
    sender_address = 'cse.takeoff@gmail.com'
    sender_pass = 'digkagfgyxcjltup'
    receiver_address = reemail
    message = MIMEMultipart()
...
def sharefilereci():
    ...
    if request.method == "POST":
        reemail = session.get('reemail', 'user@gmail.com')
        FileName = request.form['FileName']
        ...
        val = (session.get('doemail', 'owner@gmail.com'), FileName,session['reemail'], sharedrecipient, Files, otp,Date)
        print(val)
        mycursor.execute(sql, val)
        mydb.commit()
...
def Viewsharedrecipient():
    sql="select * from sharefile where reemail='%s'"%(session['reemail'])
    sql1="select * from recipientsharefile where sharedrecipient='%s'"%(session['reemail'])
    data1=pd.read_sql_query(sql,mydb)
    data2=pd.read_sql_query(sql1,mydb)
    print(data1)
    print(data2)
    mydb.commit()
    return render_template('Viewsharedrecipient.html',data1=data1, data2=data2)

@app.route('/ViewreFile/<int:id>',methods=['POST','GET'])
def ViewreFile(id):
    print(id)
    email = session.get('reemail')
    print(email)
    sql = """select FileName
    from sharefile
    WHERE reemail= %s
    UNION
    select FileName
    FROM recipientsharefile
    WHERE sharedrecipient = %s"""
    mycursor.execute(sql, (email, email))
    fdata = mycursor.fetchall()
    fdata = [j for i in fdata for j in i]
    print(fdata)
    # data = data[0][0].decode("utf-8")
    return render_template('ViewreFile.html',fdata=fdata)
...
#         print(file_key)
#         try:
#             sql = f"SELECT AES_DECRYPT(Files, aesKey) FROM filesupload WHERE FileName = '{file_name}'"
#             data = pd.read_sql_query(sql, mydb)
#             print('RRRRRRRRRRRRRRRRRRRRRRRRRR', data)
#             decrypted_content = data.values[0][0].decode('utf8')
...
#     return render_template("ViewdecriptedFile.html")
@app.route('/Filedecripted', methods=['POST', 'GET'])
def Filedecripted():
    if request.method == 'POST':
        # Get the OTP and file name from the form
        # file_key = request.form["otp"]
        file_name = request.form["FileName"]
        # print(file_key)
        print('eeeeeeeeeeeee', file_name)

        try:
            # Use parameterized query to avoid SQL injection
            sql = "SELECT AES_DECRYPT(Files, aesKey) FROM filesupload WHERE FileName = %s"
            # Execute the query with the file_name as a parameter
            data = pd.read_sql_query(sql, mydb, params=(file_name,))
            
            # Check if data is returned
            if data.empty:
                return render_template("ViewdecriptedFile.html", msg="File not found or decryption failed.")

            # Decrypt the content if data is returned
            decrypted_content = data.values[0][0].decode()
            print("Decrypted Content:", decrypted_content)

            # Render the decrypted content in the HTML template
            return render_template("ViewdecriptedFile.html", row_val=[[decrypted_content]])

        except Exception as e:
            # Handle exceptions (e.g., database errors, decryption errors)
            print("Error:", e)
            return render_template("ViewdecriptedFile.html", msg="An error occurred during decryption.")

    # Render the initial page when not a POST request
    return render_template("ViewdecriptedFile.html")
